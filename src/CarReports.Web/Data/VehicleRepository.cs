...
public sealed class VehicleRepository : IVehicleRepository
{
    ...
    public async Task<IReadOnlyList<VehicleRow>> GetVehiclesAsync(CancellationToken cancellationToken)
    {
        const string sql = """
            SELECT
                ev.id                                  AS Id,
                e.employee_code                        AS EmployeeCode,
                e.full_name_th                         AS FullNameTh,
                ev.vehicle_type                        AS VehicleType,
                ev.card_type                           AS CardType,
                LTRIM(RTRIM(ev.license_plate))         AS LicensePlate,
                ev.license_province                    AS LicenseProvince,
                ev.brand                               AS Brand,
                ev.model                               AS Model,
                ev.color                               AS Color
             ...
             ORDER BY e.employee_code, ev.vehicle_type, ev.card_type, ev.license_plate;
            """;

        using var connection = _connectionFactory.Create();
        var command = new CommandDefinition(sql, cancellationToken: cancellationToken);
        var rows = await connection.QueryAsync<VehicleRow>(command);
        return rows.ToList();
    }

    public async Task<int> UpdatePlateByIdAsync(Guid id, string newPlate, CancellationToken cancellationToken)
    {
        const string sql = """
            UPDATE dbo.employee_vehicles
               SET license_plate = @NewPlate,
                   updated_by    = @Actor,
    ...
    public async Task<int> SoftDeleteByIdAsync(Guid id, CancellationToken cancellationToken)
    {
        const string sql = """
            UPDATE dbo.employee_vehicles
               SET is_active    = 'N',
